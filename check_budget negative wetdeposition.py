...
class CalcBudget:
    ...
    def calc_budget(
        ...
        ----------
        ds : xr.Dataset
            The input dataset containing the species source and sink fields.
        ...
        time_diff_in_seconds : Union[int, None], optional
            ...
            If None, the time difference will be set to 60*60*24*30 (30 days), by default None
        verbose : bool, optional
        ...
        for field in self.source_fields.keys():
            self.source_fields[field]['value'] = (
                calculate_sum(ds[field], ['lat', 'lon']).values *
    ...
    def print_budget(
        ...
        if self.adjust_balance is not np.nan:
            print(f'{"Adjusted balance".ljust(width)}: {self.adjust_balance:.{print_precision}e} {self.unit_after_integration}')
...
def check_budget(
    ...
    for condition, name in zip([history_file, start_conditions, end_conditions], ['history_file', 'start_conditions', 'end_conditions']):
